...
$(obj)u-boot.img:	$(obj)u-boot.bin
		...
		-a $(TEXT_BASE) -e 0 \
		-n $(shell gsed -n -e 's/.*U_BOOT_VERSION//p' $(VERSION_FILE) | \
			gsed -e 's/"[	 ]*$$/ for $(BOARD) board"/') \
		-d $< $@
...
GEN_UBOOT = \
		...
		gsed  -n -e 's/.*\($(SYM_PREFIX)__u_boot_cmd_.*\)/-u\1/p'|sort|uniq`;\
...
$(obj)include/autoconf.mk: $(obj)include/config.h
	...
	$(CPP) $(CFLAGS) -DDO_DEPS_ONLY -dM include/common.h | \
		gsed -n -f tools/scripts/define2mk.sed > $@.tmp && \
	mv $@.tmp $@
...
ccardmx28js_config \
ccardmx28js_test_config \
ccardmx28js_dbg_config \
ccardwmx28js_config \
ccardwmx28js_261MHz_config \
ccardwmx28js_360MHz_config \
ccardwmx28js_test_config \
ccardwmx28js_dbg_config : unconfig
	@mkdir -p $(obj)include
	...
	@if [ "$(findstring ccardwmx28, $@)" ] ; then \
		echo "#define CONFIG_HAS_WIRELESS" >> $(obj)include/config.h ; \
		echo "Configuring a wireless module"; \
	fi;
	@$(MKCONFIG) -a ccardwmx28 arm arm926ejs ccardwmx28 freescale mx28
	@echo "CONFIG_BUILD_BOOTSTREAM = y" >> $(obj)include/config.mk
	@if [ "$(findstring _261MHz_,$@)" ]; then \
		echo "CONFIG_BOOTLETS_PATH = ccardwmx28js/261MHz" >> $(obj)include/config.mk; \
	elif [ "$(findstring _360MHz_,$@)" ]; then \
		echo "CONFIG_BOOTLETS_PATH = ccardwmx28js/360MHz" >> $(obj)include/config.mk; \
	else \
		echo "CONFIG_BOOTLETS_PATH = ccardwmx28js" >> $(obj)include/config.mk; \
	fi;
...
ccmx51js_config \
ccmx51js_128sdram_config \
ccmx51js_ext_eth_config \
ccmx51js_128sdram_ext_eth_config \
ccmx51js_test_config \
ccmx51js_128sdram_test_config \
ccmx51js_dbg_config \
ccmx51js_128sdram_dbg_config \
ccmx51js_test_dbg_config \
ccmx51js_128sdram_test_dbg_config \
ccwmx51js_config \
ccwmx51js_128sdram_config \
ccwmx51js_ext_eth_config \
ccwmx51js_128sdram_ext_eth_config \
ccwmx51js_test_config \
ccwmx51js_128sdram_test_config \
ccwmx51js_dbg_config \
ccwmx51js_128sdram_dbg_config \
ccwmx51js_128sdram_test_dbg_config \
ccwmx51js_test_dbg_config \
ccwmx51js_EAK_config \
ccwmx51js_db_pp_config \
ccwmx51js_db_pa_config \
ccwmx51js_db_rp_config \
ccwmx51js_db_ra_config \
: unconfig
	@mkdir -p $(obj)include
	@if [ "$(findstring _dbg_, $@)" ] ; then \
		echo "#define CONFIG_DOWNLOAD_BY_DEBUGGER" >> $(obj)include/config.h ; \
	...
	@if [ "$(findstring ccwmx51, $@)" ] ; then \
		echo "#define CONFIG_HAS_WIRELESS" >> $(obj)include/config.h ; \
		echo "Configuring a wireless module"; \
	fi;
	@if [ "$(findstring _ext_eth_, $@)" ] ; then \
		echo "#define CONFIG_CCXMX5X_ENABLE_EXT_ETH" >> $(obj)include/config.h ; \
		echo "Configuring for external ethernet controller"; \
	fi;
	@if [ "$(findstring _128sdram_, $@)" ] ; then \
		echo "#define CONFIG_CCXMX5_SDRAM_128MB" >> $(obj)include/config.h ; \
		echo "Configuring for 128MB of SDRAM memory"; \
	fi;
	@if [ "$(findstring _EAK_, $@)" ] ; then \
		echo "#define CONFIG_JSCCWMX51_EAK" >> $(obj)include/config.h ; \
	...
	@$(MKCONFIG) -a ccwmx51 arm arm_cortexa8 ccwmx51 freescale mx51

ccmx53js_config \
ccmx53js_128sdram_config \
ccmx53js_ext_eth_config \
ccmx53js_test_config \
ccmx53js_128sdram_test_config \
ccmx53js_dbg_config \
ccmx53js_128sdram_dbg_config \
ccmx53js_test_dbg_config \
ccmx53js_128sdram_test_dbg_config \
ccmx53js_4Kpage_config \
ccwmx53js_config \
ccwmx53js_128sdram_config \
ccwmx53js_ext_eth_config \
ccwmx53js_test_config \
ccwmx53js_128sdram_test_config \
ccwmx53js_dbg_config \
ccwmx53js_128sdram_dbg_config \
ccwmx53js_test_dbg_config \
ccwmx53js_128sdram_test_dbg_config \
ccwmx53js_4Kpage_config \
ccwmx53js_db_pp_config \
ccwmx53js_db_pa_config \
ccwmx53js_db_rp_config \
ccwmx53js_db_ra_config \
 : unconfig
	@mkdir -p $(obj)include
	@if [ "$(findstring _dbg_, $@)" ] ; then \
		echo "#define CONFIG_DOWNLOAD_BY_DEBUGGER" >> $(obj)include/config.h ; \
	...
	@$(MKCONFIG) -a ccwmx53 arm arm_cortexa8 ccwmx53 freescale mx53
