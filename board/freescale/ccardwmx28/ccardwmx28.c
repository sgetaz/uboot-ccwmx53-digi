...
#include "board-ccardxmx28.h"

DECLARE_GLOBAL_DATA_PTR;

struct ccardxmx28_hwid mod_hwid;
...
static struct pin_desc mmc0_pins_desc[] = {
	...
	/* SSP0 on ccardwmx28 only has 4 data lines
...
static int is_valid_hw_id(u8 variant)
{
	...
	if (variant < ARRAY_SIZE(ccardxmx28_id)) {
		if (ccardxmx28_id[variant].sdram != 0)
			ret = 1;
	}
	return ret;
}
...
int get_module_hw_id(void)
{
	...
	memset(&mod_hwid, 0, sizeof(struct ccardxmx28_hwid));
...
int board_init(void)
{
#if defined(CONFIG_CCARDWMX28)
	gd->bd->bi_arch_number = MACH_TYPE_CCARDWMX28JS;
#elif defined(CONFIG_CCARDMX28)
	gd->bd->bi_arch_number = MACH_TYPE_CCARDMX28JS;
#endif
...
int dram_init(void)
{
	...
	if (is_valid_hw_id(mod_hwid.variant)) {
		gd->bd->bi_dram[0].size = ccardxmx28_id[mod_hwid.variant].sdram;
	} else {
...
/* The ccardwmx28 doesn't have WP pin */
#if 0
...
int ssp_mmc_gpio_init(bd_t *bis, int index)
{
	...
	case 0:
		...
		/* The ccardwmx28 doesn't have power on and WP pins */
#if 0
...
void print_sys_info(void)
{
	...
	if (boot_verb) {
		...
		if (is_valid_hw_id(mod_hwid.variant)) {
			puts("Module HW ID      : ");
			printf("%s\n", ccardxmx28_id[mod_hwid.variant].id_string);
